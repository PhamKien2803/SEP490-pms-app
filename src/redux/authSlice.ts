  ...
  for (const module of permissionListAll) {
    ...
    for (const func of module.functions) {
      ...
      for (const act of func.actions) {
        if (act) map[url][act.name] = act.allowed;
      }
    }
  }
  return map;
...
function buildModuleMenu(permissionListAll: PermissionModule[]): ModuleMenu[] {
  ...
  return permissionListAll
    .map((module) => {
      if (!module || !Array.isArray(module.functions)) return null;
      const visibleFunctions = module.functions
        .filter(
          (func) =>
            func &&
            Array.isArray(func.actions) &&
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getCurrentUser.pending, (state) => {
        if (!state.user) state.isInitializing = true;
      })
      ...
      .addMatcher(
        (action) =>
          action.type === getCurrentUser.fulfilled.type ||
          action.type === forceRefetchUser.fulfilled.type,
        (state, action: { payload: User }) => {
          state.user = action.payload;
          state.permissionsMap = buildPermissionsMap(
      ...
      .addMatcher(
        (action) =>
          ...
          action.type === forceRefetchUser.rejected.type,
        (state) => {
          state.user = null;
          state.moduleMenu = [];
          state.permissionsMap = {};
...
export const { logout, setPermissionsStale, setToken, setUserProfile } =
  authSlice.actions;
